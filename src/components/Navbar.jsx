...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // Set initial hidden state for chip
    gsap.set(chipRef.current, { xPercent: -50, y: -120, opacity: 0, scale: 0.6 });
    ...
    const onScroll = () => {
      ...
      if (shouldCollapse) {
        // Capsule: squish inward then shoot up
        gsap.timeline()
  ...
  return (
    ...
    <div
      ...
      style={{
        ...
        boxShadow: "0 0 0 1px rgba(220,20,60,0.35), 0 8px 32px rgba(0,0,0,0.7), 0 0 20px rgba(180,0,20,0.18)",
        display: "flex",
    ...
    >
      ...
      <img src={title} alt="Megalio 2026" style={{ height: "14px", width: "auto", opacity: 0.85 }} />
    </div>

    <div
      className="fixed top-0 left-0 right-0 z-50 flex justify-center"
      style={{ paddingTop: "12px", fontFamily: "'Cinzel Decorative', 'Cinzel', serif", fontSize: "0.84rem" }}
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: "linear-gradient(to right, transparent, rgba(220,20,60,0.4), transparent)" }}
        />
        ...
        <div className="hidden md:grid grid-cols-[1fr_auto_1fr] items-center px-5 py-1">
          ...
          <ul className="flex items-center gap-6 justify-start">
            {navLeft.map((nav, i) => (
              <li key={i}>
                <NavLink to={navRoutes[nav]} className={linkClass}>{nav}</NavLink>
              </li>
            ))}
          </ul>
          ...
          <div className="flex flex-col items-center px-5 py-0.5 gap-0.5">
            ...
            <div id="title" className="flex items-center justify-center">
              <img src={title} alt="Megalio 2026" className="h-8 md:h-10 w-auto" />
            </div>
          </div>
        ...
        <div className="relative flex md:hidden items-center justify-center px-4 py-1.5">
          ...
          >
            <span className={`block h-0.5 w-6 bg-gray-300 transition-all duration-300 ${open ? "rotate-45 translate-y-2" : ""}`} />
            <span className={`block h-0.5 w-6 bg-gray-300 transition-all duration-300 ${open ? "opacity-0" : ""}`} />
            <span className={`block h-0.5 w-6 bg-gray-300 transition-all duration-300 ${open ? "-rotate-45 -translate-y-2" : ""}`} />
          </button>
        </div>
